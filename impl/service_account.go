...
func GetServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.Get, c)
	c.JSON(responseData.Code, responseData)
}

func ListServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.List, c)
	c.JSON(responseData.Code, responseData)
}

func DeleteServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.Delete, c)
	c.JSON(responseData.Code, responseData)
}

func PatchServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.Patch, c)
	c.JSON(responseData.Code, responseData)
}

func UpdateServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.Update, c)
	c.JSON(responseData.Code, responseData)
}

func CreateServiceAccount(c *gin.Context) {
	responseData := HandleServiceAccount(common.Create, c)
	c.JSON(responseData.Code, responseData)
}
...
func HandleServiceAccount(action common.ActionType, c *gin.Context) (responseData *common.ResponseData) {
	...
	r := resource.ServiceAccountResource{Params: commonParams}
	// 调用结构体方法
	switch action {
	case common.Get:
		response, err := r.Get()
