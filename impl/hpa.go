...
func GetHPA(c *gin.Context) {
	responseData := HandleHPA(common.Get, c)
	c.JSON(responseData.Code, responseData)
}

func ListHPA(c *gin.Context) {
	responseData := HandleHPA(common.List, c)
	c.JSON(responseData.Code, responseData)
}

func DeleteHPA(c *gin.Context) {
	responseData := HandleHPA(common.Delete, c)
	c.JSON(responseData.Code, responseData)
}

func PatchHPA(c *gin.Context) {
	responseData := HandleHPA(common.Patch, c)
	c.JSON(responseData.Code, responseData)
}

func UpdateHPA(c *gin.Context) {
	responseData := HandleHPA(common.Update, c)
	c.JSON(responseData.Code, responseData)
}

func CreateHPA(c *gin.Context) {
	responseData := HandleHPA(common.Create, c)
	c.JSON(responseData.Code, responseData)
}
...
func HandleHPA(action common.ActionType, c *gin.Context) (responseData *common.ResponseData) {
	...
	r := resource.HPAResource{Params: commonParams}
	// 调用结构体方法
	switch action {
	case common.Get:
		response, err := r.Get()
