...
func GetLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.Get, c)
	c.JSON(responseData.Code, responseData)
}

func ListLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.List, c)
	c.JSON(responseData.Code, responseData)
}

func DeleteLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.Delete, c)
	c.JSON(responseData.Code, responseData)
}

func PatchLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.Patch, c)
	c.JSON(responseData.Code, responseData)
}

func UpdateLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.Update, c)
	c.JSON(responseData.Code, responseData)
}

func CreateLimitRange(c *gin.Context) {
	responseData := HandleLimitRange(common.Create, c)
	c.JSON(responseData.Code, responseData)
}
...
func HandleLimitRange(action common.ActionType, c *gin.Context) (responseData *common.ResponseData) {
	...
	r := resource.LimitRangeResource{Params: commonParams}
	// 调用结构体方法
	switch action {
	case common.Get:
		response, err := r.Get()
