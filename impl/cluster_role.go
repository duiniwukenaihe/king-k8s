...
func GetClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.Get, c)
	c.JSON(responseData.Code, responseData)
}

func ListClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.List, c)
	c.JSON(responseData.Code, responseData)
}

func DeleteClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.Delete, c)
	c.JSON(responseData.Code, responseData)
}

func PatchClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.Patch, c)
	c.JSON(responseData.Code, responseData)
}

func UpdateClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.Update, c)
	c.JSON(responseData.Code, responseData)
}

func CreateClusterRole(c *gin.Context) {
	responseData := HandleClusterRole(common.Create, c)
	c.JSON(responseData.Code, responseData)
}
...
func HandleClusterRole(action common.ActionType, c *gin.Context) (responseData *common.ResponseData) {
	...
	r := resource.ClusterRoleResource{Params: commonParams}
	// 调用结构体方法
	switch action {
	case common.Get:
		response, err := r.Get()
